...
class Jira_committer:
    ...
    def createAllFromFile(self, fileName, projectIdentifier, costCenterId):

        fileReader= FileReader()
        superList = fileReader.load_content_to_objects(fileName, projectIdentifier, costCenterId)
    ...
    def createAllFromFileAndroid(self):
        ...
        for i in range(0, len(superList.subtasks)):
            listItem = superList.subtasks[i]
            fileReader.print_item_content(listItem.parent)
            issue = self.create_issue_with_fields_android(listItem.parent.name,listItem.parent.description, listItem.parent.asignee)
            for item in listItem.subtasks:
                fileReader.print_item_content(item)
                self.create_issue_with_fields_subtask_android(issue.key, item.name, item.description, item.time,item.asignee)
    ...
    def create_issue_with_fields_subtask_android(self, subtask_id,summary, description, time, assignee):
    ...
    def create_issue_with_fields_android(self, summary, description,assignee):
    ...
    def create_issue_with_fields_subtask_iphone(self,projectIdentifier, subtask_id,summary, description, time, asignee, costCenterId):
    ...
    def list_components(self):
        ...
        for component in self.authed_jira.component:
